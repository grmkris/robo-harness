...
- Netcup deployment: both systemd units (`robo-app`, `robo-rerun`) run from the `main` working tree at `/home/kristjan/code/robo-harness` — no worktree, no deploy branch. Real config and the credentials are in `/home/kristjan/.config/robo-harness.env`: since 2026-09-21 Alibaba, Grok and the scene describer all go through the cliproxy gateway with one robo key (`ROBO_ALIBABA_URL`/`ROBO_XAI_URL` → `127.0.0.1:8317`, `DASHSCOPE_API_KEY`/`XAI_API_KEY`/`ROBO_SCENE_API_KEY` = `CLIPROXY_API_KEY_ROBO`); only Jev keeps a vendor key (`AI_GATEWAY_API_KEY`, Vercel AI Gateway) (the repo `.env` is dev/mock; Bun lets the systemd env file win). Data lives in `/home/kristjan/.local/state/robo-harness` via `ROBO_DATA_DIR`. Promote with `bun run build` then `systemctl --user restart robo-app robo-rerun`.
...
Live Qwen3.8-Max camera-and-action acceptance subsequently passed with explicit operator approval: one capture, one numeric `move_joints` call, measured completion, unchanged arm joints, no errors, and released ownership. See [the acceptance report](acceptance-2026-09-08-chat-actions.md#approved-live-qwen-acceptance--passed) for operation IDs and evidence.
