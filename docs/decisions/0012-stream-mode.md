...
- The motor tick rate-limits `commanded` toward the newest setpoint at the profile's `max_speed`, so the per-tick step is bounded by the same limit an operation is bounded by. `max_step` does not apply: a stream has no discrete steps.
